/*! SW Framework 0.1.0-alpha.1 | Sandro Web Solutions | JavaScript compactado */
(function () {
  'use strict';
  if (window.SW) return;
...
(function () {
  ...
  class SWEditorInst {
    ...
    _bind() {
      ...
      const restSel = () => {
        if (!this._sel) return;
        try { const s = window.getSelection(); s.removeAllRanges(); s.addRange(this._sel); } catch (_) {  }
      };
    ...
    _updateBtns() {
      ...
      ['bold', 'italic', 'underline', 'strikeThrough', 'insertUnorderedList', 'insertOrderedList', 'justifyLeft', 'justifyCenter', 'justifyRight', 'justifyFull'].forEach((cmd) => {
        try { if (document.queryCommandState(cmd)) this.tb.querySelector(`[data-cmd="${cmd}"]`)?.classList.add('is-act'); } catch (_) {  }
      });
      ...
      try { const fb = document.queryCommandValue('formatBlock'); if (fb) this.fmt.value = fb.toUpperCase(); } catch (_) {  }
    }
